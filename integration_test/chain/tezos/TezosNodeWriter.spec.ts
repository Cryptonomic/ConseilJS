...
describe('TezosNodeWriter integration test suite', () => {
    it('Send XTZ to an account', async () => {
        ...
        const nodeResult = await TezosNodeWriter.sendTransactionOperation(tezosURL, keys, destinationAddress, 17276902, 20000);
        expect(nodeResult["operationGroupID"]).to.exist;

        const groupid = nodeResult["operationGroupID"].replace(/\"/g, '').replace(/\n/, '');
        await TezosConseilClient.awaitOperationConfirmation(conseilServer, servers.conseilNetwork, groupid, 5, 31);
    });
    ...
    it('Invoke a contract with a string literal Michelson parameter', async () => {
        ...
        const nodeResult = await TezosNodeWriter.sendContractInvocationOperation(tezosURL, keys, contractAddress, 10000, 1000, '', 1000, 100000, '"Cryptonomicon"', TezosTypes.TezosParameterFormat.Michelson);
        expect(nodeResult["operationGroupID"]).to.exist;
    ...
    it('Originate a simple contract in Michelson', async () => {
        ...
        const storage = '"Test"';
        const nodeResult = await TezosNodeWriter.sendContractOriginationOperation(tezosURL, keys, 10000, undefined, false, false, 10000, '', 10000, 10000, contract, storage, TezosTypes.TezosParameterFormat.Michelson);
        expect(nodeResult["operationGroupID"]).to.exist;

        const groupid = nodeResult["operationGroupID"].replace(/\"/g, '').replace(/\n/, '');
        await TezosConseilClient.awaitOperationConfirmation(conseilServer, servers.conseilNetwork, groupid, 5, 30);
    });
    ...
    it('Originate a simple contract in Micheline', async () => {
        ...
        const nodeResult = await TezosNodeWriter.sendContractOriginationOperation(tezosURL, keys, 10000, undefined, false, false, 10000, '', 10000, 10000, contract, storage, TezosTypes.TezosParameterFormat.Micheline);
        expect(nodeResult["operationGroupID"]).to.exist;
    ...
    it('Delegate 10xtz', async () => {
        ...
        const groupid = nodeResult["operationGroupID"].replace(/\"/g, '').replace(/\n/, '');
        const conseilResult = await TezosConseilClient.awaitOperationConfirmation(conseilServer, servers.conseilNetwork, groupid, 5, 31);
        expect(conseilResult[0].delegate).to.eq('tz1YCABRTa6H8PLKx2EtDWeCGPaKxUhNgv47');
    });

    it('Re-delegate', async () => {
        const nodeResult = await TezosNodeWriter.sendDelegationOperation(tezosURL, keys, 'KT1RniiDrmLUfTHX2JMhLJv3LDTMca1xcF2M', 'tz3WXYtyDUNL91qfiCJtVUX746QpNv5i5ve5', 10000, '');
        expect(nodeResult["operationGroupID"]).to.exist;
