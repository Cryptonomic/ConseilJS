...
export namespace SingleAssetTokenHelper {
    ...
    export async function deployContract(server: string, signer: Signer, keystore: KeyStore, fee: number, administrator: string, metadataUrl: string, pause: boolean = true, gas: number = 800_000, freight: number = 20_000): Promise<string> {
        ...
        const storage = `(Pair (Pair (Pair "${administrator}" 0) (Pair {} {Elt ""  0x${TezosMessageUtils.stringToHex(metadataUrl)}})) (Pair (Pair {} ${paused}) (Pair {} {})))`;
     ...
     * @param mapid 
     */
    export async function getTokenDefinition(server: string, mapid: number): Promise<SingleAssetTokenDefinition> {
        const packedKey = TezosMessageUtils.encodeBigMapKey(Buffer.from(TezosMessageUtils.writePackedData(0, 'nat'), 'hex'));
        const mapResult = await TezosNodeReader.getValueForBigMapKey(server, mapid, packedKey);

        if (mapResult === undefined) { throw new Error(`Map ${mapid} does not contain a record for token 0`); }
        const tokenData: SingleAssetTokenDefinition = {
            tokenid: Number(JSONPath({ path: '$.args[0].int', json: mapResult })[0]),
            metadata: {}
        };
        mapResult["args"][1].forEach(item => {
            tokenData.metadata[item["args"][0]["string"]] = TezosMessageUtils.hexToString(item["args"][1]["bytes"])
        });
        return tokenData;
    }
    ...
    export async function mint(server: string, address: string, signer: Signer, keystore: KeyStore, fee: number, destination: string, amount: number, metadata: Record<string, string>, gas: number = 800_000, freight: number = 20_000): Promise<string> {
        ...
        for (let key of keys) {
            ...
            metaString += `Elt "${key}" 0x${TezosMessageUtils.stringToHex(metadata[key])}`
        }
