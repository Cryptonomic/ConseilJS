...
export namespace TezosOperations {
  ...
  export async function sendOperation(
    ...
    const forgedOperationGroup = await forgeOperations(
      network,
      blockHead,
      operations
    );
    const signedOpGroup = await signOperationGroup(
      forgedOperationGroup,
      keyStore,
      derivationPath
    );
    const operationGroupHash = computeOperationHash(signedOpGroup);
    const appliedOp = await applyOperation(
      network,
      blockHead,
      operations,
      operationGroupHash,
  ...
  export async function appendRevealOperation(
    ...
    const isManagerKeyRevealed = await isManagerKeyRevealedForAccount(
      network,
      keyStore
    );
  ...
  export async function sendContractOriginationOperation(
    ...
    const origination: ContractOriginationOperation = {
      ...
      fee: fee.toString(),
      counter: (Number(account.counter) + 1).toString(),
      gas_limit: gas_limit === '' ? '10160' : gas_limit,
      storage_limit: storage_limit === '' ? '277' : storage_limit,
      //   managerPubkey: keyStore.publicKeyHash, // mainnet, alphanet
      manager_pubkey: keyStore.publicKeyHash, // zeronet
      balance: amount.toString(),
      spendable: spendable,
   ...
   */
  export async function isImplicitAndEmpty(
    network: string,
    accountHash: string
  ): Promise<boolean> {
    const blockHead = await TezosNode.getBlockHead(network);
   ...
   * @returns {Promise<boolean>}  Result
   */
  export async function isManagerKeyRevealedForAccount(
    network: string,
    keyStore: KeyStore
  ): Promise<boolean> {
    const blockHead = await TezosNode.getBlockHead(network);
    const managerKey = await TezosNode.getAccountManagerForBlock(
      network,
      blockHead.hash,
      keyStore.publicKeyHash
    );
