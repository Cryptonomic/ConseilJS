...
export namespace TezFns {
    ...
    function trezorQuery(id, data, nocb?) {
        ...
        return new Promise((resolve, reject) => {
          ...
          for(let i = 0; i < packets.length; i++){
            outep.transfer([63].concat(packets[i]), (e) => {if (e) console.log("err", e)});
          }
        });
    }

    export function sign(path, branch, operation, revealOp) {
		return new Promise((resolve, reject) => {
			openDevice().then(() => {
				load().then(() => {
					const tx: any = {
    ...
    export function getAddress (path, isShow = false) {
		return new Promise((resolve, reject) => {
			openDevice().then(() => {
				load().then(() => {
